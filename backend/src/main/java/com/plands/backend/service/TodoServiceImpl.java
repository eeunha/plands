...
public class TodoServiceImpl implements TodoService {
    ...
    public boolean createTodo(TodoRequestDto todoRequestDto) {
        ...
        if (todoRequestDto.getMemberPlantIds() != null && !todoRequestDto.getMemberPlantIds().isEmpty()) {
            ...
            for (Long memberPlantId : todoRequestDto.getMemberPlantIds()) {
                ...
                if (insertedRows <= 0) {
                    System.out.println("등록 실패: 식물 매핑 인서트 중 오류 발생 (memberPlantId = " + memberPlantId + ")");
                    return false;
                }
            }
        }
    ...
    public boolean updateTodo(Long todoId,  TodoRequestDto todoRequestDto) {
        ...
        if (affectedRows <= 0) {
            System.out.println("수정 실패: 존재하지 않는 투두 ID = " + todoId);
            return false;
        }
        ...
        if (todoRequestDto.getMemberPlantIds() != null && !todoRequestDto.getMemberPlantIds().isEmpty()) {
            for (Long memberPlantId : todoRequestDto.getMemberPlantIds()) {
                ...
                if (insertRows <= 0) {
                    System.out.println("수정 실패: 식물 매핑 인서트 중 오류 발생 (memberPlantId = " + memberPlantId + ")");
                    return false;
                }
            }
        }
